...
public class BoardDaoimpl implements BoardDao {
	...
	private class BoardPreparedStatementSetterForPrimarykey implements PreparedStatementSetter{
		private Integer seq;
		public BoardPreparedStatementSetterForPrimarykey(int seq) {
			super(); this.seq=seq;
		}
		@Override
	...
	private class UserResultSetExtractor implements ResultSetExtractor{
		...
		public Object extractData(ResultSet rs)throws SQLException,DataAccessException {
			//직접 하나 하나 처리해야함 rowmapper는 스프링이 반복해서 처리해줌.
			if(rs.next()) {
				BoardDto dto = new BoardDto();
				dto.setSeq(rs.getInt("seq"));
				dto.setTitle(rs.getString("title"));
			...
			throw new DataRetrievalFailureException("해당 객체를 찾을수 없습니다.");
		}
	}
	@Override
